...
import { ArrowLeft, ArrowRight, Clock, Share2, Bookmark, ExternalLink } from 'lucide-react';
import { useLanguage } from '../contexts/LanguageContext';
import { useAuth } from '../contexts/AuthContext';

// Reusing the enhanced ContentRenderer from CaseDetail for consistency
...
const NewsDetail: React.FC = () => {
  ...
  // Find article in all non-case collections
  const newsItem = [...data.reports, ...data.methodologies, ...data.announcements].find(n => n.slug === slug);

  if (!isAuthenticated) {
    return <Navigate to={`/gate?redirect=/news/${slug}`} replace />;
  }

  if (!newsItem) {
    return <div className="text-center py-20 text-white">Article not found</div>;
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 mb-24">
        ...
        <div className="lg:col-span-8 animate-fade-in">
           ...
           <div className="mb-10 space-y-6">
            <div className="flex flex-wrap gap-2">
               {newsItem.tags.map(tag => (
                  <Badge key={tag} className="bg-white/5 text-slate-200 border-white/10">{tag}</Badge>
               ))}
            </div>
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-semibold text-white leading-[1.1] tracking-tight">{newsItem.title}</h1>
            <p className="text-xl text-slate-400 font-light leading-relaxed max-w-2xl">{newsItem.subtitle}</p>
            ...
            <div className="flex items-center gap-6 text-xs text-slate-500 border-t border-white/10 pt-6 font-mono uppercase tracking-wide">
              <span className="flex items-center gap-2"><Clock size={14} /> {newsItem.readTime} {t('readTime')}</span>
              <span>{t('updated')} {newsItem.updatedAt}</span>
            </div>
           </div>
           ...
           <div className="prose prose-invert prose-lg max-w-none prose-headings:text-white prose-p:text-slate-300 prose-strong:text-white">
             {newsItem.contentBlocks.map((block, idx) => (
        ...
        <div className="lg:col-span-4 space-y-6 animate-fade-in animation-delay-200">
           <div className="sticky top-24 space-y-4">
              ...
              {/* Card 3: Actions */}
              <div className="bg-white/5 border border-white/10 rounded-xl p-5 backdrop-blur-xl space-y-3">
                 <div className="grid grid-cols-2 gap-3">
                    <button className="h-10 rounded-full flex items-center justify-center text-slate-300 hover:text-white hover:bg-white/5 border border-white/5 hover:border-white/20 text-sm gap-2 transition-all font-medium">
      ...
      <div className="pt-12 border-t border-white/10 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[...data.reports, ...data.methodologies].filter(n => n.slug !== slug).slice(0, 3).map(item => (
            <GlassCard key={item.slug} className="group cursor-pointer border-white/5 hover:border-white/10 transition-all duration-300" hoverEffect={true} noPadding={true}>
               <div onClick={() => navigate(`/news/${item.slug}`)} className="flex items-center p-4 gap-4">
                 ...
                 <div>
                    <h4 className="font-bold text-white text-sm mb-1 group-hover:text-primary-400 transition-colors line-clamp-2 leading-snug">{item.title}</h4>
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider">{item.tags[0]}</span>
                 </div>
               </div>
            </GlassCard>
          ))}
